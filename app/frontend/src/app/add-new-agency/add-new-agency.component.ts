...
@Component({
  selector: 'app-add-new-agency',
  templateUrl: './add-new-agency.component.html',
  styleUrls: ['./add-new-agency.component.css']
})
export class AddNewAgencyComponent implements OnInit {
            ...
            if (res != null) {
              ...
              // 8) Check identification format:
                ...
                this.http.post('http://localhost:4000/upload', this.fd).subscribe((result : string) => {
                  ...
                  this.usersService.addAgency(this.username, this.password, this.phone, this.mail, photoname, 
                    this.name, this.country, this.city, this.street, this.identification, this.description).subscribe(resp=>{
